  ...
  return (
    <main>
      ...
      <section className="hero" id="inicio">
        ...
        <div className="hero-visual" aria-label="Representação de uma estrutura industrial em aço">
          <div className="visual-haze" />
          <div className="steel-frame frame-one"><i /><i /><i /><i /></div>
          <div className="steel-frame frame-two"><i /><i /><i /><i /></div>
          <div className="steel-frame frame-three"><i /><i /><i /><i /></div>
          <div className="visual-label"><span>Projetos sob medida</span><strong>PR</strong></div>
        </div>
      </section>
      ...
      <section className="intro" id="solucoes">
        <div><div className="section-kicker">01 — Nossas soluções</div><h2>Soluções completas para sua construção.</h2></div>
      ...
      <section className="services-grid" aria-label="Serviços em destaque">
        {services.slice(0, 6).map((service) => (
          <article className="service-card" key={service.number}>
            ...
            <div className="card-symbol" aria-hidden="true"><span /><span /></div>
            <h3>{service.title}</h3>
            <p>{service.short}</p>
      ...
      <section className="made-to-order">
        <div className="order-copy"><div className="section-kicker light-kicker">02 — Engenharia aplicada</div><h2>Seu projeto.<br />Nossa estrutura.</h2><p>Do barracão à estrutura metálica, dos fechamentos aos componentes complementares: produzimos cada solução pensando na realidade da sua operação.</p><a className="button button-primary" href={whatsappUrl} target="_blank" rel="noreferrer">Conversar sobre meu projeto <span>↗</span></a></div>
        <div className="order-art" aria-hidden="true"><span>SOB</span><strong>MEDIDA</strong><i>01</i><i>02</i><i>03</i><i>04</i></div>
      </section>

      <section className="differentials" id="empresa">
        <div className="differentials-head"><div className="section-kicker">03 — Por que a DiuPav</div><h2>Compromisso que sustenta grandes ideias.</h2></div>
        <div className="difference-list">
          {differentials.map((item, index) => <div key={item}><span>{String(index + 1).padStart(2, '0')}</span><strong>{item}</strong><i aria-hidden="true">+</i></div>)}
        </div>
      </section>

      <section className="about-band">
        <div className="about-number"><strong>5</strong><span>anos de<br />reconhecimento</span></div>
        <div className="about-copy"><div className="section-kicker light-kicker">04 — Experiência para construir resultados</div><h2>Uma empresa movida por desafios.</h2><p>Profissionais experientes, comprometidos e motivados a gerar resultados positivos e oferecer soluções que agreguem valor aos clientes, parceiros e à comunidade.</p><a className="text-link light-link" href="/sobre">Conheça a DiuPav Barracões <span>↗</span></a></div>
      </section>
